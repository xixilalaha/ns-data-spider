...
async function requestUSATarget3() {
  function getAllJson() {
    return new Promise((resolve, reject) => {
  ...
  //   console.log(end, 'end')
  //   let resultData = await formatHtml(tempData.slice(start, end))
  //   await writeJson(resultData, `NUSA${i}.json`)
  // }
  for (let i = 1; i <= count; i++) {
    let data = await readFileJson(i)
    let idxArr = data.map(item => {
      return item.idx
    })
    let errArr = data.filter(item => {
      return "content" in item
    })
    if(errArr.length){
      console.log(i,'errArr')
      console.log(errArr.length,'errArr')
    }
    ...
    // }
    // if(errArr.length){
    //   await writeJson(data, `NUSA${i}.json`)
    // }
  }
  let resultData = ['数据爬取成功']
  return resultData
}
...
function readFileJson2(i) {
  return new Promise((resolve, reject) => {
    jsonfile.readFile(`./dist/NErrorUSA${i}.json`)
      .then(obj => resolve(obj))
      .catch(error => reject(error))
  })
}
